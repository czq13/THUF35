...
//static RC_Channel_aux *_aux_channels[8];///添加////
RC_Channel_aux *RC_Channel_aux::_aux_channels[RC_AUX_MAX_CHANNELS];///被注释，原来的,RC_AUX_MAX_CHANNELS代替8//////
...
{
    for (uint8_t i = 0; i < RC_AUX_MAX_CHANNELS; i++) {                //RC_AUX_MAX_CHANNELS代替8////
        if (_aux_channels[i]) {
